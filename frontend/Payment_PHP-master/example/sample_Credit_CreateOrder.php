...
*/
    ...
    $sql2 = 'SELECT * FROM order_details WHERE customer_orderID='.$_POST['customer_orderID'];
    ...
    try {
        ...
        foreach($order_details as $order_detail){
            array_push($obj->Send['Items'], array('Name' => $order_detail['name'], 'Price' => (int)$order_detail['price'],
                   'Currency' => "元", 'Quantity' => (int)$order_detail['quantity'], 'URL' => "dedwed"));
        }
        

        //Credit信用卡分期付款延伸參數(可依系統需求選擇是否代入)
        //以下參數不可以跟信用卡定期定額參數一起設定
        $obj->SendExtend['OrderResultURL'] = 'http://localhost/happy_cake_4/frontend/payment_success.php';
